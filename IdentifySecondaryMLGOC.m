function handles = IdentifySecondaryMLGOC(handles)

% Help for the Identify Secondary MLGOC module:
% Category: Object Processing
%
% SHORT DESCRIPTION:
% Identifies objects (e.g. cell edges) using "seed" objects identified by
% an Identify Primary module or Identify Primary MLGOC module (e.g. nuclei)
% so it can handle overlapping objects.
% *************************************************************************
%
...
% Extended by the Biological Research Center in Szeged
% Copyright 2015.
%
%
...
LabelMatrixImage = CPretrieveimage(handles,['Segmented', PrimaryObjectName],ModuleName,'DontCheckColor','DontCheckScale');
...
%     PrelimPrimaryLabelMatrixImage = CPretrieveimage(handles,['SmallRemovedSegmented', PrimaryObjectName],ModuleName,'DontCheckColor','DontCheckScale',size(OrigImage));
    ...
    for l=1:NumberOfLayers
        Perim(:,:,l) = bwperim(FinalSecondaryLabels(:,:,l)>0);
    end
    
    SecondaryOutline = any(Perim, 3);
    
    ObjectOutlinesOnOrigImage(SecondaryOutline>0) = 1;
    ...
    PrimaryOutline = any(PrimaryPerim, 3);
    
    %%% Storing both primary and secondary contours
    BothOutlinesOnOrigImage = ObjectOutlinesOnOrigImage;
    BothOutlinesOnOrigImage(PrimaryOutline) = 1;
    ...
    try
        if ~strcmpi(SaveOutlines,'Do not save')
            handles.Pipeline.(SaveOutlines) = SecondaryOutline;
        end
    catch
        error(['The object outlines were not calculated by the ', ModuleName, ' module, so these images were not saved to the handles structure. The Save Images module will therefore not function on these images. This is just for your information - image processing is still in progress, but the Save Images module will fail if you attempted to save these images.'])
    end

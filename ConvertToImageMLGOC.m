function handles = ConvertToImageMLGOC(handles)
...
if NumberOfLayers==1
    ...
    Image = cell(1,NumberOfLayers);
    
    for l=1:NumberOfLayers
        
        LabelMatrixImage = squeeze(tempImage(:,:,l));
        
        if strcmp(ImageMode,'Binary (black & white)')
            Image{l} = logical(LabelMatrixImage ~= 0);
        elseif strcmp(ImageMode,'Grayscale')
            warning off Matlab:DivideByZero
            Image{l} = double(LabelMatrixImage / max(max(LabelMatrixImage)));
            warning on Matlab:DivideByZero
        elseif strcmp(ImageMode,'Color')
            if strcmpi(ColorMap,'Default')
                Image{l} = CPlabel2rgb(handles,LabelMatrixImage);
            else
                try
                    cmap = eval([ColorMap '(max(max(2,LabelMatrixImage(:))))']);
                catch
                ...
                Image{l} = label2rgb(LabelMatrixImage,cmap,'k');
            end
        elseif strcmp(ImageMode,'uint16')
            Image{l} = uint16(LabelMatrixImage);
        end
